class Gui3DManager {
    ...
    create3dLegend (scene, elements, options){
        ...
        elements.forEach(element => {
            ...
            //Check width of text for given font type at any size of font
            var ctx = dynamicTexture.getContext();
            var size = 800; //any value will work
            ctx.font = size + "px " + font_type;
            var textWidth = ctx.measureText(text).width;
            ...
            //set font to be actually used to write text on dynamic texture
            var font_size = Math.floor(DTWidth / (ratio * .05)); //size of multiplier (1) can be adjusted, increase for smaller text
            // var font = font_size + "px " + font_type;
            var font = ctx.font;
            ...
            if (options.horizontalLabels){
                ...
                // plane1.position.z = -planeWidth/2 - .5;
                plane1.position.x = 0;
                plane1.position.y = yPos2 * planeHeight;
                ...
                element.userData.my3DLabel = plane1;
                plane1.userData = {};
                plane1.userData.myElement = element;
...
// console.log('text',text1)
    ...
    create3DText2(scene, scale, depth, displayText, xPos, yPos, zPos, color){
        // var  MeshWriter, text1, text2, C1, C2;
